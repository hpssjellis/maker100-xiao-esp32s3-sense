...
// AES256-CTR decryption function
void decryptAES256CTR(uint8_t* ciphertext, size_t len, uint8_t* key, uint8_t* iv, uint8_t* plaintext) {
  AES256 aes256;
  CTR<AES256> ctr;
  ctr.setKey(key, aes256.keySize());
  ctr.setIV(iv, aes256.blockSize());
...
void receivePacket() {
  ...
  if (state == RADIOLIB_ERR_NONE) {
    ...
    uint8_t key[32] = { /* Your AES key here */ };
    uint8_t iv[16] = { /* Your IV here */ };
    uint8_t ciphertext[str.length()];
    uint8_t plaintext[str.length()];
    str.getBytes(ciphertext, str.length());
    decryptAES256CTR(ciphertext, str.length(), key, iv, plaintext);
